...
> {
  ...
  onSubmit = async () => {
    ...
    if (!name) return this.setState({ errorName: "Veuillez saisir votre nom" });
    if (!fname)
    ...
    return (
      ...
      >
        <Input
          ...
          onChange={e =>
            this.setState({ name: this.capitalizeFirstLetter(e.target.value) })
          }
        ...
        <Input
          ...
          onSubmit={this.onSubmit}
          onChange={e =>
            this.setState({ fname: this.capitalizeFirstLetter(e.target.value) })
          }
...
class SendVerifView extends React.Component<
  SendVerifViewProps,
  SendVerifViewState
> {
    ...
    return (
      ...
      >
        <Input
          ...
          clearable
        />
        <Button onClick={this.onSubmit}>{this.props.text}</Button>
      </div>
    );
  }
}
...
class LoginScreen extends React.Component<LoginScreenProps, LoginScreenState> {
  ...
  secondStage = user => {
    if (user.id && user.name && user.fname) this.props.history.push("/");
    else this.setState({ secondDone: true });
  };
